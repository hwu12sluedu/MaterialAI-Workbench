...
from material_ai_workbench.abaqus_batch_client import AbaqusBatchConfig, DEFAULT_SMAPYTHON
...
from material_ai_workbench.rve_visualization import plot_oriented_fiber_rve_3d, plot_rve_3d_from_run, plot_rve_3d
...
from material_ai_workbench.nl_tasks import parse_natural_language_task, task_from_dict, task_to_workbench_config
...
def _ensure_llm_ui_defaults() -> None:
    if st.session_state.get("llm_ui_initialized"):
        return
    cfg = llm_config_from_env()
    st.session_state["llm_provider"] = cfg.provider_name if cfg.provider_name in LLM_PROVIDER_PRESETS else "custom"
...
def _llm_config_wizard() -> tuple[LlmChatConfig, str, bool, bool]:
    ...
    c0, c1 = st.columns(2)
    apply_preset_clicked = c0.button("套用提供商预设", use_container_width=True, key="llm_apply_provider")
    test_clicked = c1.button("测试连接", use_container_width=True, key="llm_test_connection")
    if apply_preset_clicked:
        ...
        st.session_state["llm_api_key_env"] = str(preset.get("api_key_env", "MATERIALAI_LLM_API_KEY"))
        st.session_state["llm_require_key"] = bool(preset.get("require_api_key", True))
        st.session_state["llm_last_connection_ok"] = False
        st.rerun()

    llm_base_url = st.text_input("Base URL", placeholder="例如 https://api.openai.com/v1", key="llm_base_url")
    llm_model = st.text_input("模型", placeholder="例如 gpt-4.1-mini / deepseek-chat / qwen-plus", key="llm_model")
...
def _ai_task_panel() -> None:
    ...
    with right:
        ...
        if task.abaqus.run_check and not allow_abaqus:
            st.info("已识别到 Abaqus 验算需求。勾选“允许本次任务调用 Abaqus”后，执行任务才会真正提交 Abaqus。")
        ...
        if execute_clicked:
            ...
            with st.spinner("正在执行自然语言任务：材料训练..."):
                result = _run_material_training_with_feedback(config, "正在训练材料模型...")
            if result is None:
                return
...
def _execute_composite_ai_task(payload: dict[str, Any], *, allow_abaqus: bool) -> None:
    ...
    cols[4].metric("Kt 估算", _fmt_metric(result.engineering_estimates.get("stress_concentration_factor_estimate")))
...
def _composite_config_from_ai_payload(payload: dict[str, Any], *, allow_abaqus: bool) -> CompositePlateConfig:
    composite = payload.get("composite") if isinstance(payload.get("composite"), dict) else {}
    ...
    return CompositePlateConfig(
        ...
        fiber_volume_fraction=_payload_float(composite, "fiber_volume_fraction", 0.55, lower=0.05, upper=0.8),
        ...
        interface_efficiency=_payload_float(composite, "interface_efficiency", 0.92, lower=0.1, upper=1.5),
        interface_thickness_ratio=_payload_float(composite, "interface_thickness_ratio", 0.18, lower=0.0, upper=0.8),
        ...
        micro_fiber_count=_payload_int(composite, "micro_fiber_count", 16, lower=1, upper=200),
...
def _payload_float(data: dict[str, Any], key: str, default: float, *, lower: float | None = None, upper: float | None = None) -> float:
    try:
...
def _payload_int(data: dict[str, Any], key: str, default: int, *, lower: int | None = None, upper: int | None = None) -> int:
...
def _training_panel() -> None:
    ...
    with left:
            ...
            if recommend_clicked:
                if not allow_recommendation:
                    ...
                    if recommendation is None:
                        ...
                        st.session_state["train_E"] = float(params.get("youngs_modulus", st.session_state.get("train_E", 200_000.0)))
                        st.session_state["train_nu"] = float(params.get("poisson_ratio", st.session_state.get("train_nu", 0.3)))
                        st.session_state["train_sy"] = float(params.get("yield_strength", st.session_state.get("train_sy", 60.0)))
                        st.session_state["train_C"] = float(params.get("c_value", st.session_state.get("train_C", 1.0)))
                        st.session_state["train_gamma"] = float(params.get("gamma", st.session_state.get("train_gamma", 1.0)))
                        st.session_state["train_n_load_cases"] = int(params.get("n_load_cases", st.session_state.get("train_n_load_cases", 40)))
                        st.session_state["train_n_sequence"] = int(params.get("n_sequence", st.session_state.get("train_n_sequence", 4)))
                        ratios = params.get("hill_ratios") or []
                        if len(ratios) == 6:
                            for idx, value in enumerate(ratios):
                                st.session_state[f"train_hill_r{idx + 1}"] = max(MIN_HILL_RATIO, float(value))
        ...
        youngs_modulus = c1.number_input("E (MPa)", min_value=1.0, step=1000.0, key="train_E")
        poisson_ratio = c2.number_input("nu", min_value=0.0, max_value=0.49, step=0.01, key="train_nu")
        ...
        st.divider()
        hill_ratios = tuple(st.session_state.get(f"train_hill_r{i + 1}", value) for i, value in enumerate(HILL_RATIO_DEFAULTS))
            ...
            with st.expander("Hill 各向异性比例", expanded=True):
                ...
                hill_ratios = tuple(
                    hcols[i].number_input(
                        ...
                        max_value=10.0,
                        value=float(st.session_state.get(f"train_hill_r{i + 1}", HILL_RATIO_DEFAULTS[i])),
                        step=0.05,
                        key=f"train_hill_r{i + 1}",
                    )
                    for i in range(6)
                )
        barlat_alphas = tuple(st.session_state.get(f"train_barlat_a{i + 1}", value) for i, value in enumerate(BARLAT_ALPHA_DEFAULTS))
                ...
                for i, default in enumerate(BARLAT_ALPHA_DEFAULTS):
                    alpha_values.append(
                        alpha_cols[i % 4].number_input(
                            ...
                            min_value=0.05,
                            max_value=5.0,
                            value=float(st.session_state.get(f"train_barlat_a{i + 1}", default)),
                            step=0.05,
        ...
        if material_type in {"neo_hookean", "mooney_rivlin"}:
            with st.expander("超弹性参数", expanded=True):
                ...
                hyperelastic_c10 = h1.number_input("C10 (MPa)", min_value=1.0e-6, value=hyperelastic_c10, step=0.1, key="train_hyperelastic_c10")
                ...
                hyperelastic_d1 = h3.number_input("D1", min_value=0.0, value=hyperelastic_d1, step=0.001, format="%.6f", key="train_hyperelastic_d1")
                st.caption("超弹性分支生成应力-应变曲线和 Abaqus *HYPERELASTIC 材料卡，不走 SVC 屈服面训练。")

        st.divider()
        with st.expander("训练设置", expanded=True):
            c1, c2 = st.columns(2)
            c_value = c1.number_input("SVC C", min_value=0.01, step=0.5, key="train_C")
            gamma = c2.number_input("SVC gamma", min_value=0.01, step=0.25, key="train_gamma")

            c3, c4, c5 = st.columns(3)
            n_load_cases = c3.number_input("载荷方向数", min_value=8, step=8, key="train_n_load_cases")
            n_sequence = c4.number_input("采样序列", min_value=2, step=1, key="train_n_sequence")
            test_size = c5.number_input("测试点数", min_value=20, step=20, key="train_test_size")

            c6, c7 = st.columns(2)
            plot_mesh = c6.number_input("屈服面网格", min_value=20, step=10, key="train_plot_mesh")
            calculate_curves = c7.checkbox("同时计算 pyLabFEA 曲线", key="train_calculate_curves")

        run_clicked = st.button("开始训练", type="primary", use_container_width=True, key="train_run")
    ...
    with right:
        ...
        if run_clicked:
            if material_type == "hill" and any(float(value) <= 0 for value in hill_ratios):
                st.error("Hill 各向异性比例必须全部为正数。请使用默认模板或输入大于 0 的比例。")
                return
            if material_type == "barlat" and any(float(value) <= 0 for value in barlat_alphas):
...
def _data_import_panel(selected_run: Path | None) -> None:
    ...
    with left:
        ...
        source_kind = st.selectbox(
            ...
            format_func=lambda item: "实验应力-应变曲线" if item == "experiment_curve" else "Abaqus 批量结果 CSV",
        )
        material_name = st.text_input("材料/数据集名称", value="imported_material")
        uploaded = st.file_uploader("上传 CSV", type=["csv", "txt"])
        import_upload_clicked = st.button("导入上传文件", type="primary", use_container_width=True, key="import_upload")
        ...
        if abaqus_csv:
            ...
            import_abaqus_clicked = st.button("导入当前 Abaqus CSV", use_container_width=True, key="import_current_abaqus_csv")
        else:
    ...
    with right:
        ...
        if import_abaqus_clicked and abaqus_csv:
            with st.spinner("正在导入当前 Abaqus 结果 CSV..."):
                result = import_csv_dataset(
                    ...
                    material_name=(selected_run.name if selected_run else material_name.strip() or "abaqus_result"),
                )
            result_dir = result.import_dir
            st.session_state["selected_import_dir"] = str(result_dir)
        ...
        if imports:
            ...
            chosen = st.selectbox("选择已导入数据", options, index=default_index, format_func=lambda item: Path(item).name)
            ...
            if curve_config is None:
                ...
                cols[1].metric("估算 sy (MPa)", _fmt_metric(curve_config.yield_strength))
                cols[2].metric("模型", curve_config.material_type.upper())
                confirm_curve_train = st.checkbox("确认使用该曲线估算参数训练材料模型", value=False, key="import_confirm_train_curve")
                ...
                if train_from_import_clicked:
                    result = _run_material_training_with_feedback(curve_config, "正在根据导入曲线训练材料模型...")
                ...
                validation_material = validation_cols[0].selectbox(
                    "验证材料模型",
                    ["j2", "hill", "barlat"],
                    format_func=lambda item: {"j2": "J2 各向同性", "hill": "Hill 各向异性", "barlat": "Barlat 近似"}[item],
                    key="import_validation_material",
                )
                validation_cases = validation_cols[1].number_input("训练载荷数", min_value=8, max_value=120, value=24, step=4, key="import_validation_cases")
                validation_tests = validation_cols[2].number_input("测试点数", min_value=20, max_value=300, value=40, step=10, key="import_validation_tests")
                ...
                if validate_curve_clicked:
                    try:
                        ...
                        metric_cols[1].metric("MAE (MPa)", _fmt_metric(validation.mean_abs_error_mpa))
                        metric_cols[2].metric("Max Error (MPa)", _fmt_metric(validation.max_abs_error_mpa))
                        st.image(str(validation.overlay_plot), caption="实验曲线 vs Workbench 生成曲线")
...
def _case_library_panel() -> None:
    ...
    with left:
        ...
        source_folder = st.text_input("Abaqus 案例文件夹或单个 .inp 文件", value=default_folder, key="case_source_folder")
        title = st.text_input("案例标题", value="Abaqus 仿真案例", key="case_title")
        tags = st.text_input("标签", value="Abaqus, 成功案例", key="case_tags")
        status = st.selectbox("状态", ["success", "candidate", "failed", "needs_review"], index=0, key="case_status")
        ...
        next_actions = st.text_area("后续动作", value="", height=70, key="case_next_actions")
        scan_clicked = st.button("扫描并归档案例", type="primary", use_container_width=True, key="case_scan_archive")
        ...
        with st.expander("批量导入 + 重复检测", expanded=False):
            ...
            batch_parent = st.text_input("父目录", value="", placeholder="包含多个 Abaqus 案例的目录", key="case_batch_parent")
            skip_dup = st.checkbox("跳过已有案例", value=True, key="case_batch_skip_dup")
            batch_tags = st.text_input("批量标签", value="", placeholder="逗号分隔，例如 j2, 批量导入", key="case_batch_tags")
                ...
                if not batch_parent.strip():
                    ...
                    with st.spinner(f"正在批量扫描 {batch_parent} ..."):
                        try:
                            ...
                            if dup_check:
                                st.warning(f"检测到 {len(dup_check)} 个可能重复的案例（精确路径或文件名/大小匹配）")
                                with st.expander("重复详情", expanded=True):
                                    st.dataframe([{
                                        "Case ID": d["case_id"], "Title": d["title"],
                                        "Match": d["match_type"], "Status": d["status"],
                                    } for d in dup_check], use_container_width=True, hide_index=True, height=200)
                            tag_list = [t.strip() for t in batch_tags.split(",") if t.strip()]
                            result = batch_import_cases(batch_parent, tags=tag_list, skip_existing=bool(skip_dup))
                            st.success(f"导入完成：{result['imported']} 新增, {result['skipped']} 跳过, {result['failed']} 失败")
        ...
        st.divider()
        st.markdown("**训练数据集**")
        dataset_name = st.text_input("数据集名称", value="case_dataset", key="case_dataset_name")
        export_dataset_clicked = st.button("导出案例库训练数据集", use_container_width=True, key="case_export_dataset")
        ...
        with st.expander("批量导出向导", expanded=False):
            ...
            if not wizard_cases:
                ...
                status_options = sorted({case.status for case in wizard_cases if case.status})
                ...
                selected_statuses = st.multiselect("状态", status_options, default=status_options, key="case_export_status_filter")
                selected_materials = st.multiselect("材料模型", material_options, default=[], key="case_export_material_filter")
                selected_case_types = st.multiselect(
                    "案例类型",
                    ["metal", "composite", "unknown"],
                    default=[],
                    format_func=lambda item: {"metal": "金属/塑性", "composite": "复合材料/RVE", "unknown": "未分类"}[item],
                    key="case_export_type_filter",
                )
                tag_filter = st.text_input("标签/关键词", value="", key="case_export_tag_filter")
                d1, d2 = st.columns(2)
                date_from = d1.text_input("起始日期", value="", placeholder="YYYY-MM-DD", key="case_export_date_from")
                date_to = d2.text_input("结束日期", value="", placeholder="YYYY-MM-DD", key="case_export_date_to")
                ...
                if matched_cases:
                    ...
                    st.dataframe(preview_rows, use_container_width=True, hide_index=True, height=260)
                filtered_export_name = st.text_input("筛选数据集名称", value=f"{dataset_name}_filtered", key="case_filtered_dataset_name")
                ...
                if export_filtered_clicked:
                    try:
                        export = export_case_dataset(
                            name=filtered_export_name.strip() or "case_dataset_filtered",
    ...
    with right:
        ...
        st.dataframe(case_table_rows(cases), use_container_width=True, hide_index=True, height=400)
...
def _show_case_summary(summary: Any) -> None:
    ...
    if inp_rows:
        ...
        feature_cols[2].metric("单元(估算)", aggregate.get("estimated_element_count", 0))
    ...
    if similar_rows:
        ...
        st.dataframe(display_rows, use_container_width=True, hide_index=True, height=260)
...
def _case_odb_extraction_panel(summary: Any) -> None:
    ...
    if extraction_rows:
        ...
        st.dataframe(extraction_rows, use_container_width=True, hide_index=True, height=400)
    ...
    with st.expander("提取 ODB 场变量", expanded=False):
        ...
        c1, c2, c3 = st.columns(3)
        host = c1.text_input("MCP host", value=str(st.session_state.get("mcp_host", "127.0.0.1")), key=f"case_mcp_host_{summary.case_id}")
        port = c2.number_input("MCP port", min_value=1, max_value=65535, value=int(st.session_state.get("mcp_port", 48152)), step=1, key=f"case_mcp_port_{summary.case_id}")
        timeout_seconds = c3.number_input("超时(秒)", min_value=5.0, max_value=300.0, value=30.0, step=5.0, key=f"case_mcp_timeout_{summary.case_id}")
        c4, c5 = st.columns(2)
        ...
        capture_contour = c5.checkbox("提取后抓取 S-Mises 云图", value=True, key=f"case_odb_capture_{summary.case_id}")
        skip_existing = st.checkbox("批量时跳过已有 ODB 提取记录", value=True, key=f"case_odb_skip_existing_{summary.case_id}")
        b1, b2 = st.columns(2)
        extract_clicked = b1.button("提取当前 ODB", type="primary", use_container_width=True, key=f"case_odb_extract_{summary.case_id}")
        batch_extract_clicked = b2.button("批量提取全部 ODB", use_container_width=True, key=f"case_odb_batch_extract_{summary.case_id}")

        if extract_clicked or batch_extract_clicked:
            fields = [item.strip() for item in fields_text.replace("，", ",").split(",") if item.strip()]
            config = AbaqusMcpConfig(host=host.strip() or "127.0.0.1", port=int(port), timeout_seconds=float(timeout_seconds))
            batch_config = AbaqusBatchConfig(abaqus_python=Path(batch_python), timeout_seconds=float(timeout_seconds) * 10)
            ...
            if batch_extract_clicked and skip_existing:
                ...
                targets = [path for path in targets if str(Path(path).expanduser().resolve()) not in existing_paths]
            if not targets:
            ...
            try:
                with st.spinner("正在读取 ODB 最后一帧场变量..."):
                    ...
                    for target_odb in targets:
                        extraction = run_case_odb_extraction(
                            ...
                            capture_contour=bool(capture_contour) and not batch_extract_clicked,
                            backend=backend,
                            config=config,
                ...
                st.json([{"odb": item.get("odb_name"), "backend": item.get("backend_used"), **item.get("aggregate", {})} for item in completed])
                first_image = next((item.get("viewport_image") for item in completed if item.get("viewport_image")), None)
                if first_image and Path(first_image).exists():
                    st.image(first_image, caption="ODB S-Mises 云图", use_container_width=True)
                st.rerun()
            except Exception as exc:
                st.error(f"ODB 场变量提取失败：{exc}")
                st.info("实时 MCP 需要 Abaqus/CAE 已打开并启动 Socket Bridge；批处理方式需要本机 SMAPython.exe 路径有效。")
    ...
    with st.expander("提取 ODB 帧曲线", expanded=False):
        ...
        series_clicked = sb1.button("提取当前 ODB 曲线", type="primary", use_container_width=True, key=f"case_odb_series_extract_{summary.case_id}")
        series_batch_clicked = sb2.button("批量提取全部 ODB 曲线", use_container_width=True, key=f"case_odb_series_batch_{summary.case_id}")

        if series_clicked or series_batch_clicked:
            fields = [item.strip() for item in series_fields_text.replace("，", ",").split(",") if item.strip()]
            region_names = [item.strip() for item in series_regions_text.replace("，", ",").split(",") if item.strip()]
            batch_config = AbaqusBatchConfig(abaqus_python=Path(series_python), timeout_seconds=max(float(series_max_frames), 60.0) * 5)
            targets = list(odb_options) if series_batch_clicked else [selected_series_odb]
            ...
            try:
                ...
                st.info("帧曲线使用 Abaqus SMAPython.exe 批处理读取 ODB，请确认路径有效且 ODB 未被其他进程锁定。")
...
def _case_odb_options(summary: Any) -> list[str]:
    ...
    for path in ((summary.result_features or {}).get("summary", {}) or {}).get("odb_files", []):
        if path and Path(path).exists():
            text = str(Path(path))
    ...
    if source.is_dir():
        for path in sorted(source.rglob("*.odb"), key=lambda item: item.stat().st_mtime, reverse=True):
            text = str(path)
            if text not in options:
                options.append(text)
...
def _material_library_controls() -> None:
    ...
    with st.expander("材料库", expanded=False):
        if presets:
            ...
            if st.button("加载到训练页", use_container_width=True, key="library_load_to_training"):
        ...
        if st.button("保存当前参数", use_container_width=True, key="library_save_current"):
            preset_name = st.session_state.get("library_save_name") or st.session_state.get("train_name", "material_preset")
...
def _abaqus_mcp_panel(selected_run: Path | None) -> None:
    ...
    with left:
        ...
        port = st.number_input("Bridge 端口", min_value=1, max_value=65535, value=48152, step=1, key="mcp_port")
        ...
        c0, c1 = st.columns(2)
        check_clicked = c0.button("检查连接", type="primary", use_container_width=True, key="mcp_check")
        stop_clicked = c1.button("停止 Bridge", use_container_width=True, key="mcp_stop")
        snapshot_clicked = st.button("生成会话快照", use_container_width=True, key="mcp_snapshot")

        st.divider()
        st.markdown("**工作目录**")
        default_workdir = selected_run / "abaqus_verification" if selected_run else REPO_ROOT
        workdir_text = st.text_input("Abaqus 当前工作目录", value=str(default_workdir), key="mcp_workdir")
        set_workdir_clicked = st.button("设置 Abaqus 工作目录", use_container_width=True, key="mcp_set_workdir")
    ...
    with right:
        ...
        if stop_clicked:
            try:
                ...
                st.success("Bridge 停止请求已发送。重新加载插件或重启 Abaqus/CAE 后，再启动 Socket Bridge。")
                st.json(result)
        ...
        if status_data:
            if status_data.get("connected"):
                ...
                cols[3].metric("Processed", (telemetry.get("bridge") or {}).get("processed", "N/A"))
        ...
        st.markdown("**模型与 Job**")
        c1, c2, c3 = st.columns(3)
        model_clicked = c1.button("读取模型", use_container_width=True, key="mcp_read_model")
        jobs_clicked = c2.button("读取 Job", use_container_width=True, key="mcp_read_jobs")
        monitor_clicked = c3.button("监控 Job", use_container_width=True, key="mcp_monitor_job")
        ...
        selected_job = st.selectbox("选择 Job", job_names or [""], format_func=lambda item: item or "暂无 Job")
        confirm_submit = st.checkbox("确认允许提交选中的 Abaqus Job", value=False)
        submit_clicked = st.button("提交并等待 Job 完成", disabled=not bool(selected_job and confirm_submit), use_container_width=True, key="mcp_submit_job")

        if monitor_clicked:
            try:
                st.json(monitor_job_status(selected_job if selected_job else "", config))
            except Exception as exc:
        ...
        selected_odb = st.selectbox("本地 ODB", odb_options or [""], format_func=lambda item: Path(item).name if item else "暂无 ODB")
        odb_path = st.text_input("ODB 路径", value=selected_odb or default_odb, key="mcp_odb_path")
        c4, c5 = st.columns(2)
        inspect_odb_clicked = c4.button("读取 ODB 元数据", use_container_width=True, key="mcp_inspect_odb")
        capture_clicked = c5.button("抓取当前视口", use_container_width=True, key="mcp_capture_viewport")
        ...
        if inspect_odb_clicked:
            if not odb_path.strip():
                ...
                try:
                    st.session_state["mcp_odb_info"] = inspect_odb(Path(odb_path), config)
                except Exception as exc:
        ...
        if capture_clicked:
            output_dir = (selected_run / "abaqus_verification" / "mcp_viewports") if selected_run else (REPO_ROOT / "material_ai_workbench" / "mcp_sessions")
            try:
        ...
        if snapshot_clicked:
            with st.spinner("正在生成 Abaqus MCP 会话快照..."):
                snapshot = create_session_snapshot(selected_run=selected_run, config=config, capture_image=True)
            st.success("快照已生成")
            st.caption(f"目录: `{snapshot.snapshot_dir}`")
            if snapshot.viewport_path and snapshot.viewport_path.exists():
                st.image(str(snapshot.viewport_path), caption="会话快照视口", use_container_width=True)
...
def _abaqus_panel(selected_run: Path | None) -> None:
    ...
    with left:
        ...
        max_load_cases = st.number_input("载荷工况数", min_value=0, max_value=9, value=1, step=1)
        timeout_seconds = st.number_input("超时 (秒)", min_value=60, max_value=7200, value=1200, step=60)
        ...
        c1, c2 = st.columns(2)
        prepare_clicked = c1.button("准备目录", use_container_width=True, key="abaqus_prepare")
        run_clicked = c2.button("运行 Abaqus", type="primary", use_container_width=True, key="abaqus_run")
        st.divider()
    ...
    with right:
        ...
        if (prepare_clicked or run_clicked) and (abaqus_bat_path is None or str(abaqus_bat_path) == "." or not abaqus_bat_path.exists()):
...
def _abaqus_job_queue_panel(default_run_dir: Path | None) -> None:
    ...
    default_work_dir = str((Path(default_input).parent if default_input else (default_run_dir or REPO_ROOT)).resolve())
    ...
    if input_options:
        ...
        selected_input = ""
    input_file = st.text_input("Input File (.inp)", value=selected_input or default_input, key="queue_input_file")
    work_dir = st.text_input("工作目录", value=default_work_dir, key="queue_work_dir")
    q1, q2 = st.columns(2)
    default_job_name = (Path(input_file).stem if input_file and input_file.strip() else "") or "queued_abaqus_job"
    job_name = q1.text_input(
        "Job 名称",
        value=default_job_name,
        key="queue_job_name",
    )
    cpus = q2.number_input("CPUs", min_value=1, max_value=64, value=4, step=1, key="queue_cpus")
    timeout_seconds = st.number_input("单个 Job 超时 (秒)", min_value=60, max_value=86400, value=7200, step=300, key="queue_timeout")

    queue = JobQueue(abaqus_bat=Path(queue_abaqus))
    b1, b2, b3 = st.columns(3)
    submit_clicked = b1.button("加入队列", type="primary", use_container_width=True, key="queue_submit")
    process_clicked = b2.button("处理下一个", use_container_width=True, key="queue_process_next")
    clear_clicked = b3.button("清理已完成", use_container_width=True, key="queue_clear_completed")
    ...
    stat_cols[2].metric("平均耗时(s)", _fmt_metric(status.get("average_duration_seconds")))
    rows = []
            ...
            with st.expander(f"{job.name} / {job.status}", expanded=False):
                ...
                if retry_clicked:
                    try:
                        ...
                        st.error(f"重试失败：{exc}")
                if c2.button("查看日志", use_container_width=True, key=f"queue_log_{job.job_id}"):
...
def _composite_panel() -> None:
    ...
    st.caption("生成 Fiber/Interface/Matrix 三相微观体素 RVE、机器学习 phase map、pyLabFEA 材料摘要和 Abaqus 3D 带孔板验证脚本。")
    composite_root = REPO_ROOT / COMPOSITE_ROOT
    left, right = st.columns([0.38, 0.62], gap="large")

    with left:
        st.markdown("**微观材料与 RVE**")
        name = st.text_input("案例名称", value="micro_rve_plate_hole", key="composite_name")
        st.caption("纤维属性")
        c1, c2 = st.columns(2)
        vf = c1.number_input("纤维体积分数", min_value=0.05, max_value=0.8, value=0.55, step=0.01, key="composite_vf")
        interface_efficiency = c2.number_input("界面效率", min_value=0.1, max_value=1.5, value=0.92, step=0.01, key="composite_eta")
        c3, c4 = st.columns(2)
        fiber_e = c3.number_input("纤维 E (MPa)", min_value=1_000.0, value=230_000.0, step=5_000.0, key="composite_fiber_e")
        fiber_nu = c4.number_input("纤维 nu", min_value=0.0, max_value=0.49, value=0.20, step=0.01, key="composite_fiber_nu")
        st.caption("基体属性")
        c5, c6 = st.columns(2)
        matrix_e = c5.number_input("基体 E (MPa)", min_value=100.0, value=3_500.0, step=100.0, key="composite_matrix_e")
        matrix_nu = c6.number_input("基体 nu", min_value=0.0, max_value=0.49, value=0.35, step=0.01, key="composite_matrix_nu")
        st.caption("界面与 RVE 网格")
        c7, c8 = st.columns(2)
        interface_ratio = c7.number_input("界面厚度/纤维半径", min_value=0.0, max_value=0.8, value=0.18, step=0.02, key="composite_interface_ratio")
        micro_fibers = c8.number_input("RVE 纤维数", min_value=1, max_value=100, value=16, step=1, key="composite_micro_fibers")
        c9, c10, c11 = st.columns(3)
        micro_nx = c9.number_input("nx", min_value=2, max_value=40, value=8, step=1, key="composite_micro_nx")
        micro_ny = c10.number_input("ny", min_value=2, max_value=80, value=18, step=1, key="composite_micro_ny")
        micro_nz = c11.number_input("nz", min_value=2, max_value=80, value=18, step=1, key="composite_micro_nz")

        st.divider()
        st.markdown("**纤维取向与几何**")
        fo1, fo2 = st.columns(2)
        fiber_theta = fo1.number_input("纤维角度 θ (°)", min_value=0.0, max_value=90.0, value=0.0, step=1.0, key="composite_theta",
                                       help="纤维绕 Z 轴的主方向角，0 = 沿 X 方向")
        fiber_spread = fo2.number_input("取向分散 (°)", min_value=0.0, max_value=30.0, value=8.0, step=0.5, key="composite_spread",
                                        help="纤维角度的标准差，越大纤维越随机")
        fo3, fo4 = st.columns(2)
        fiber_len = fo3.number_input("纤维长度 (相对 RVE)", min_value=0.3, max_value=3.0, value=1.2, step=0.1, key="composite_fiber_len")
        fiber_diam = fo4.number_input("纤维直径 (相对 RVE)", min_value=0.01, max_value=0.3, value=0.12, step=0.01, key="composite_fiber_diam",
                                      help="0 表示由 Vf 自动校准")
        ...
        st.caption("宏观几何与加载")
        p1, p2 = st.columns(2)
        length = p1.number_input("长度 L (mm)", min_value=10.0, value=120.0, step=5.0, key="composite_length")
        width = p2.number_input("宽度 W (mm)", min_value=5.0, value=40.0, step=2.0, key="composite_width")
        p3, p4 = st.columns(2)
        thickness = p3.number_input("厚度 T (mm)", min_value=0.1, value=2.0, step=0.1, key="composite_thickness")
        hole_radius = p4.number_input("孔半径 R (mm)", min_value=0.1, value=5.0, step=0.5, key="composite_hole")
        p5, p6 = st.columns(2)
        applied_strain = p5.number_input("拉伸应变", min_value=0.0001, value=0.003, step=0.0005, format="%.5f", key="composite_strain")
        mesh_size = p6.number_input("宏观网格尺寸", min_value=0.2, value=2.0, step=0.2, key="composite_mesh")
        st.caption("Abaqus 与随机设置")
        p7, p8 = st.columns(2)
        cpus = p7.number_input("Abaqus CPUs", min_value=1, max_value=32, value=4, step=1, key="composite_cpus")
        random_seed = p8.number_input("随机种子", min_value=1, max_value=99999, value=7, step=1, key="composite_seed")
        run_abaqus = st.checkbox("生成后立即调用 Abaqus 建模", value=False, key="composite_run_abaqus")
        submit_job_now = st.checkbox("Abaqus 建模后提交求解 Job", value=False, key="composite_submit_job", disabled=not run_abaqus)
        create_clicked = st.button("生成复合材料闭环案例", type="primary", use_container_width=True, key="composite_create_case")
    ...
    with right:
        with st.expander("微观纤维取向 3D 预览 (可旋转/缩放)", expanded=True):
            ...
            if "rve_preview_key" not in st.session_state or preview_key != st.session_state.get("rve_preview_key"):
                with st.spinner("正在生成纤维 RVE 3D 预览..."):
                    preview_config = CompositePlateConfig(
                        ...
                        fiber_e=float(fiber_e), fiber_nu=float(fiber_nu),
                        matrix_e=float(matrix_e), matrix_nu=float(matrix_nu),
                        micro_fiber_count=int(micro_fibers),
                        micro_nx=int(micro_nx), micro_ny=int(micro_ny), micro_nz=int(micro_nz),
                        ...
                        fiber_diameter_normalized=float(fiber_diam) if float(fiber_diam) > 0 else None,
                    )
                    try:
                        layout = preview_config if False else None  # just a marker
                        st.session_state["rve_preview_fig"] = plot_oriented_fiber_rve_3d(
                            config=preview_config, width=700, height=550,
                        )
                        st.session_state["rve_preview_key"] = preview_key
                    except Exception:
                        st.info("plotly 未安装。运行 `pip install plotly` 后即可查看 3D 纤维微观结构。")
                        st.session_state["rve_preview_fig"] = None
                        st.session_state["rve_preview_key"] = preview_key
            if st.session_state.get("rve_preview_fig") is not None:
                st.plotly_chart(st.session_state["rve_preview_fig"], use_container_width=True, key="rve_3d_chart")
        ...
        if create_clicked:
            try:
                ...
                with st.spinner("正在生成微观 RVE、phase map、pyLabFEA 摘要和 Abaqus 脚本..."):
                    result = run_composite_plate_workflow(config)
                st.session_state["selected_composite_run"] = str(result.run_dir)
        ...
        cols[2].metric("Kt 估算", _fmt_metric(estimates.get("stress_concentration_factor_estimate")))
        cols[3].metric("RVE 单元", _fmt_metric(micro.get("micro_voxel_elements")))
        cols[4].metric("界面单元", _fmt_metric(micro.get("micro_rve_interface_elements")))

        img_left, img_right = st.columns(2)
        _show_image(img_left, _resolve_output_path(paths.get("microstructure_png")), "三相微观 RVE 预览")
        _show_image(img_right, _resolve_output_path(paths.get("plate_preview_png")), "3D 带孔板验证模型预览")
        ...
        file_rows = [
            ...
            {"artifact": "six PBC run script", "path": paths.get("micro_pbc_run_script")},
            {"artifact": "RVE stiffness postprocess", "path": paths.get("micro_pbc_postprocess_script")},
            {"artifact": "pyLabFEA material summary", "path": paths.get("pylabfea_material_summary")},
        ...
        if dataset_csv and dataset_csv.exists():
            with st.expander("训练数据行预览", expanded=False):
                st.dataframe(_read_csv_preview(dataset_csv), use_container_width=True, hide_index=True, height=400)
        ...
        with st.expander("复合材料批量数据", expanded=False):
            b1, b2, b3 = st.columns(3)
            batch_name = b1.text_input("批量计划名", value="composite_rve_sweep", key="composite_batch_name")
            sample_count = b2.number_input("样本数", min_value=1, max_value=200, value=6, step=1, key="composite_batch_samples")
            max_run_samples = b3.number_input("本次运行样本", min_value=1, max_value=50, value=2, step=1, key="composite_batch_max_run")
            r1, r2 = st.columns(2)
            vf_min = r1.number_input("Vf min", min_value=0.05, max_value=0.8, value=0.35, step=0.01, key="composite_batch_vf_min")
            vf_max = r2.number_input("Vf max", min_value=0.05, max_value=0.8, value=0.65, step=0.01, key="composite_batch_vf_max")
            create_batch_clicked = st.button("创建复合材料批量计划", use_container_width=True, key="create_composite_batch")
            ...
            if plans:
                ...
                run_batch_clicked = st.button("运行批量样本", use_container_width=True, key="run_composite_batch")
                if run_batch_clicked:
                    try:
                        with st.spinner("正在生成复合材料批量样本..."):
                            plan = run_composite_batch_plan(chosen_plan, max_samples=int(max_run_samples))
                        st.success("批量样本生成完成")
                    except Exception as exc:
                    ...
                    plan = load_composite_batch_plan(chosen_plan)
                st.dataframe(plan.data.get("samples", []), use_container_width=True, hide_index=True, height=400)
                if plan.dataset_csv.exists() and plan.dataset_csv.stat().st_size > 0:
                    st.caption(f"`{plan.dataset_csv}`")
                    st.dataframe(_read_csv_preview(plan.dataset_csv), use_container_width=True, hide_index=True, height=400)
            else:
                st.info("还没有复合材料批量计划。")

        with st.expander("复合材料代理模型", expanded=False):
            plans = list_composite_batch_plans(REPO_ROOT / COMPOSITE_BATCH_ROOT)
            dataset_options = [str(load_composite_batch_plan(path).dataset_csv) for path in plans if load_composite_batch_plan(path).dataset_csv.exists()]
            dataset_options = [path for path in dataset_options if Path(path).stat().st_size > 0]
            if dataset_options:
                ...
                target_column = st.selectbox(
                    "预测目标",
                    ["max_stress_near_hole_estimate_mpa", "E1", "E2", "G12", "stress_concentration_factor_estimate"],
                    key="composite_surrogate_target",
                )
                model_kind = st.segmented_control(
                    "模型类型",
                    options=["random_forest", "mlp"],
                    format_func=lambda item: "随机森林" if item == "random_forest" else "神经网络 MLP",
                    key="composite_surrogate_model_kind",
                )
                train_composite_clicked = st.button("训练复合材料代理模型", use_container_width=True, key="train_composite_surrogate")
                if train_composite_clicked:
                    try:
                        with st.spinner("正在训练复合材料代理模型..."):
                            surrogate = train_composite_surrogate(selected_dataset, target_column=target_column, model_kind=model_kind)
                        st.success(f"代理模型已生成：{surrogate.run_dir.name}")
                        st.json(surrogate.metrics)
                    except Exception as exc:
                        st.error(f"训练复合材料代理模型失败：{exc}")
                composite_comparison = composite_surrogate_comparison_rows(dataset_csv=selected_dataset, target_column=target_column)
                _show_surrogate_comparison(composite_comparison, title="复合材料代理模型对比")
                all_composite_runs = list_composite_surrogate_runs()
                if all_composite_runs:
                    st.caption(f"复合材料代理模型历史：`{len(all_composite_runs)}` 个 run")
...
def _batch_panel() -> None:
    st.subheader("批量仿真")
    st.caption("把单个闭环样本扩展为多样本训练集。第一版先做材料参数扫描和串行队列，Abaqus 求解保持显式确认。")
    left, right = st.columns([0.38, 0.62], gap="large")
    ...
    with left:
        ...
        material_type = st.segmented_control(
            "材料类型",
            options=["j2", "hill"],
            format_func=lambda item: "J2 isotropic" if item == "j2" else "Hill anisotropic",
            key="batch_material_type",
        )
        sy_text = st.text_input("屈服强度列表 MPa", value="50,60,70", key="batch_sy_values")
        c1, c2 = st.columns(2)
        youngs_modulus = c1.number_input("E (MPa)", min_value=1.0, value=200_000.0, step=1000.0, key="batch_E")
        poisson_ratio = c2.number_input("nu", min_value=0.0, max_value=0.49, value=0.3, step=0.01, key="batch_nu")
        c3, c4 = st.columns(2)
        n_load_cases = c3.number_input("训练载荷方向数", min_value=8, value=32, step=8, key="batch_n_load_cases")
        n_sequence = c4.number_input("采样序列", min_value=2, value=3, step=1, key="batch_n_sequence")
        max_abaqus_load_cases = st.number_input("每个样本 Abaqus 载荷工况", min_value=1, max_value=9, value=1, step=1, key="batch_abaqus_cases")
        create_clicked = st.button("创建批量计划", type="primary", use_container_width=True, key="batch_create_plan")

        st.divider()
        st.markdown("**运行控制**")
        allow_abaqus = st.checkbox("允许本次批量任务调用 Abaqus", value=False, key="batch_allow_abaqus")
        archive_cases = st.checkbox("运行后写入案例库", value=False, key="batch_archive_cases")
        postprocess_odb = st.checkbox("写入案例库后提取 ODB", value=False, key="batch_postprocess_odb")
        max_samples = st.number_input("本次最多处理样本数", min_value=1, value=1, step=1, key="batch_max_samples")
        run_material_clicked = st.button("只运行材料训练", use_container_width=True, key="batch_run_material")
        run_abaqus_clicked = st.button("运行材料训练 + Abaqus", disabled=not allow_abaqus, use_container_width=True, key="batch_run_abaqus")
...
def _surrogate_panel() -> None:
    st.subheader("代理模型")
    st.caption("把案例库导出的 Abaqus/ODB 特征表训练成快速预测模型，用于后续自然语言仿真的初筛和抽样回验。")
    left, right = st.columns([0.38, 0.62], gap="large")
    ...
    with left:
        ...
        if datasets:
            ...
            model_kind = st.segmented_control(
                ...
                format_func=lambda item: {"random_forest": "随机森林", "mlp": "神经网络 MLP", "gbr": "梯度提升 GBR"}[item],
            ...
            train_clicked = st.button("训练代理模型", type="primary", use_container_width=True, key="surrogate_train")
            compare_clicked = st.button("多模型对比 (RF vs MLP vs GBR)", use_container_width=True, key="surrogate_compare_all")
            st.caption("样本量很少时，这一步主要验证流程；真正的预测能力来自后续持续积累的 Abaqus 案例库。")

            if compare_clicked:
                with st.spinner("正在训练三种模型并生成对比报告..."):
                    try:
                        comparison = compare_all_models(selected_dataset, target_column=target_column)
                        _show_surrogate_comparison(comparison, title="多模型对比 (同数据集)")
                        st.success("三种模型训练完成，对比表如上。")
                    except Exception as exc:
                        st.error(f"多模型对比失败：{exc}")

            with st.expander("ODB 帧曲线时序代理", expanded=False):
                default_frame_index = str(Path(selected_dataset) / "frame_series_index.csv")
                ts_case_dataset = st.text_input("case_dataset.csv", value=selected_dataset, key="ts_case_dataset")
                ts_frame_index = st.text_input("frame_series_index.csv", value=default_frame_index, key="ts_frame_index")
                t1, t2, t3 = st.columns(3)
                ts_field = t1.text_input("场变量", value="S", key="ts_target_field")
                ts_metric = t2.selectbox("曲线指标", ["max", "mean", "min"], key="ts_target_metric")
                ts_model_kind = t3.selectbox("模型", ["random_forest", "mlp"], key="ts_model_kind")
                ts_train_clicked = st.button("训练时序代理模型", use_container_width=True, key="ts_train_surrogate")
            with st.expander("多保真代理模型", expanded=False):
                ...
                mf_train_clicked = st.button("训练多保真代理模型", use_container_width=True, key="mf_train_surrogate")
        else:
            selected_dataset = ""
        ...
        closed_loop_clicked = st.button("生成最新闭环报告", use_container_width=True, key="surrogate_closed_loop_report")
    ...
    with right:
        ...
        if closed_loop_clicked:
            try:
                with st.spinner("正在汇总材料训练、Abaqus、案例库、数据集和代理模型..."):
        ...
        if ts_train_clicked:
            try:
                ...
                if ts_result.get("error"):
                    ...
                    if plot_path.exists():
                        st.image(str(plot_path), caption="时序曲线预测对比", use_container_width=True)
            except Exception as exc:
        ...
        if mf_train_clicked:
            try:
                ...
                X_high, y_high, high_features = _dataset_to_xy(Path(high_dataset), mf_target, feature_names=features)
                ...
                if mf_result.plot_path.exists():
                    st.image(str(mf_result.plot_path), caption="多保真 vs 单保真", use_container_width=True)
            except Exception as exc:
                st.error(f"多保真代理模型训练失败：{exc}")

        if datasets:
            comparison = surrogate_comparison_rows(dataset_dir=selected_dataset, target_column=target_column)
...
def _management_panel(selected_run: Path | None) -> None:
    ...
    with material_col:
        ...
        presets = load_material_presets()
        if presets:
            st.dataframe(_material_table_rows(presets), use_container_width=True, hide_index=True, height=400)
            selected = st.selectbox("选择材料模板", list(presets), key="manager_material_name")
            c1, c2 = st.columns(2)
            if c1.button("加载到训练页", use_container_width=True, key="manager_load_material"):
                for key, value in preset_to_training_state(presets[selected]).items():
                    st.session_state[key] = value
                st.success(f"已加载：{selected}")
            if c2.button("删除模板", use_container_width=True, key="manager_delete_material"):
                delete_material_preset(selected)
                st.success(f"已删除：{selected}")
                st.rerun()
            st.divider()
            st.markdown("**一键 Abaqus 验算**")
            allow_library_abaqus = st.checkbox("允许本次调用 Abaqus", value=False, key="manager_allow_library_abaqus")
            load_cases = st.number_input("验算载荷工况数", min_value=1, max_value=9, value=1, step=1, key="manager_library_abaqus_cases")
            ...
            if quick_verify_clicked:
                ...
                try:
                    ...
                    with st.spinner("正在从材料库模板训练材料模型..."):
                        trained = _run_material_training_with_feedback(config, "正在从材料库模板训练材料模型...")
...
def _run_selector(label: str, default_run: Path | None = None) -> Path | None:
    ...
    options = [str(path) for path in runs]
    if "selected_run_dir" in st.session_state and st.session_state["selected_run_dir"] in options:
        ...
        index = 0
    selected = st.selectbox(label, options, index=index, format_func=lambda item: Path(item).name)
...
def _ensure_training_defaults() -> None:
    ...
    defaults.update({f"train_hill_r{i + 1}": value for i, value in enumerate(HILL_RATIO_DEFAULTS)})
    defaults.update({f"train_barlat_a{i + 1}": value for i, value in enumerate(BARLAT_ALPHA_DEFAULTS)})
...
def _show_run_summary(run_dir: Path) -> None:
    ...
    _show_image(fig_cols[0], _resolve_output_path(outputs.get("yield_locus_png")), "屈服面")
    _show_image(fig_cols[1], _resolve_output_path(outputs.get("stress_strain_png")), "应力-应变")
...
def _show_abaqus_summary(work_dir: Path) -> None:
    ...
    if plot_path and plot_path.exists():
        st.image(str(plot_path), caption="Abaqus 单元验算曲线", use_container_width=True)
    ...
    if result_csv and result_csv.exists():
        with st.expander("结果 CSV 预览", expanded=False):
            st.dataframe(_read_csv_preview(result_csv), use_container_width=True, hide_index=True, height=400)
...
def _show_import_summary(import_dir: Path) -> None:
    ...
    if normalized_csv.exists():
        with st.expander("标准化曲线预览", expanded=False):
            st.dataframe(read_normalized_preview(normalized_csv), use_container_width=True, hide_index=True, height=400)
...
def _list_runs() -> list[Path]:
    if not RUNS_ROOT.exists():
        return []
    runs = [path for path in RUNS_ROOT.iterdir() if path.is_dir() and (path / "summary.json").exists()]
...
def _find_abaqus_result_csv(run_dir: Path | None) -> Path | None:
    ...
    if not results_dir.exists():
        return None
    candidates = sorted(results_dir.glob("*-res.csv"), key=lambda path: path.stat().st_mtime, reverse=True)
...
def _find_odb_candidates(run_dir: Path | None) -> list[Path]:
    ...
    if run_dir:
        ...
        if abaqus_dir.exists():
            _add(sorted(abaqus_dir.rglob("*.odb"), key=lambda p: p.stat().st_mtime, reverse=True))
    ...
    if run_dir:
        ...
        if runs_root.name == "runs" and runs_root.exists():
            for odb_path in sorted(runs_root.rglob("*.odb"), key=lambda p: p.stat().st_mtime, reverse=True):
    ...
    try:
        ...
        for case in list_cases():
            ...
            if source.is_dir():
                for odb_path in sorted(source.rglob("*.odb"), key=lambda p: p.stat().st_mtime, reverse=True):
                    key = str(odb_path.resolve())
                    if key not in seen:
                        seen.add(key)
                        candidates.append(odb_path)
...
def _find_inp_candidates(run_dir: Path | None) -> list[Path]:
    ...
    for root in roots:
        if root is None or not Path(root).exists():
            continue
        for inp_path in sorted(Path(root).rglob("*.inp"), key=lambda path: path.stat().st_mtime, reverse=True):
...
def _run_table_rows() -> list[dict[str, Any]]:
    ...
    for run_dir in _list_runs():
        ...
        try:
            ...
            LOGGER.warning("Skipping unreadable run summary: %s (%s)", summary_path, exc)
            continue
...
def _dataset_to_xy(dataset_dir: Path, target_column: str, feature_names: list[str] | None = None):
    ...
    if len(clean_rows) < 2:
        raise ValueError(f"At least two rows with numeric {target_column} are required.")
    ...
    if not feature_names:
        raise ValueError("No numeric feature columns found for multi-fidelity training.")
...
def _show_surrogate_run(run_dir: Path) -> None:
    ...
    st.markdown(f"**Run**: `{run_dir.name}`")
    cols = st.columns(5)
    cols[0].metric("目标", _surrogate_target_label(str(metrics.get("target_column", ""))))
    ...
    if plot_path.exists():
        st.image(str(plot_path), caption="预测值 vs Abaqus 真值", use_container_width=True)

    predictions_csv = run_dir / "predictions.csv"
    if predictions_csv.exists():
        with st.expander("预测明细 CSV", expanded=False):
            st.dataframe(_read_csv_preview(predictions_csv), use_container_width=True, hide_index=True, height=400)
...
def _show_surrogate_comparison(rows: list[dict[str, Any]], *, title: str = "代理模型对比") -> None:
    st.markdown(f"**{title}**")
    if not rows:
    ...
    best = rows[0]
    st.success(f"当前最佳：`{best.get('model_kind', '')}`，RMSE={_fmt_metric(best.get('rmse'))}，run：`{best.get('run', '')}`。")
    chart_rows = [
...
def _show_batch_plan(plan: Any) -> None:
    ...
    st.dataframe(batch_sample_table_rows(plan), use_container_width=True, hide_index=True, height=400)
    trend_path = _resolve_output_path((data.get("outputs") or {}).get("batch_trend_png"))
